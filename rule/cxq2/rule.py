...
def read_train():
#    读取训练集
    user_label = {'user_id':[],'label':[],'day_count':[]}
    ...
    for i,file in enumerate(tr_files):
       ...
       user_label['day_count'].extend(list(df['day_count'] ))
    df = pd.DataFrame(user_label,columns=['user_id','label','day_count'])
    df.to_csv('train_userid_label_day.csv',index=False,header=False)
    
def deal_train():
    df = pd.read_csv('train_userid_label_day.csv',names=['user_id','label','day_count'])
    #只保留在某个类别呆的天数超过一半的用户
    print (len(df))
