...
parser.add_argument('--dataset_root', default = 'data_txt', type = str)
...
parser.add_argument('--basenet', default = 'MultiModal', type = str)
...
diag_prec = [0.7764877595700922, 0.7009188902632975, 0.7586477987421384, 0.45641646489104115,0.8807205452775073, 0.7352978696262278, 0.5521264584117426, 0.4470659407138536,0.5774774774774775]
...
def main():
    ...
    print("load pretrained model from model 2 _____adam2")
    state_dict1 = torch.load('model2/BDXJTU2019_SGD_12_26000.pth')
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        #if is_best:
        if epoch%1 == 0:
            1#torch.save(model.state_dict(), 'model2/BDXJTU2019_SGD_' + repr(epoch) + '.pth')
...
def train(Dataloader,model, criterion, optimizer, epoch):
    ...
    for i, (input_img, input_vis, anos) in enumerate(Dataloader):
        ...
        top1.update(prec1, input_img.size(0))
        top5.update(prec5, input_img.size(0))
        ...
        if (i+1)%2000==0:
            torch.save(model.state_dict(), 'model2/BDXJTU2019_SGD_' + repr(epoch)+"_"+str(int(i+1))+ '.pth')
        if (i+1) % 200 == 0:
            print('Epoch: [{0}][{1}/{2}]\t'
