...
def test(test_loader, model, fold, ckpt, best_what, if_gen_txt):
    ...
    for i, (image, visit, _) in tqdm(enumerate(test_loader)):
        with torch.no_grad():
            ...
            #print(y_pred.shape,"y_pred",preds_temp.shape)
    ...
    save_name = '%s_val_%.4f_fold%d_%s_TTA.npy'%(ckpt["model_name"], ckpt["best_acc"], fold, best_what)
    save_path = os.path.join(save_dir, save_name)
    np.save(save_path, pred_npy)
    np.save(os.path.join(save_dir, "csv71.npy"), pred_csv_npy)
    if if_gen_txt:
...
def training(train_data_list, val_data_list, test_files, fold):
    ...
    test_gen = MultiModalDataset(test_files, config.test_data, config.test_vis, augument=False, mode="tta")
    test_loader = DataLoader(test_gen, 128, shuffle=False, pin_memory=True, num_workers=4)
    ...
    checkpoint_loss = torch.load('checkpoints/best_models/0626_debug_fold_0_model_best_loss.pth.tar')
    model.load_state_dict(checkpoint_loss["state_dict"])
    test(test_loader, model, fold, checkpoint_loss, 'best_loss', False)

    checkpoint_acc = torch.load('checkpoints/best_models/0626_debug_fold_0_model_best_acc.pth.tar')
