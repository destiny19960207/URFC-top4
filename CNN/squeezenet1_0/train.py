...
log = Logger()
log.open(("logs/%s_log_train_"+str(config.start_fold)+".txt")%config.model_name, mode="a")
...
def train(train_loader,model,criterion,optimizer,epoch,valid_metrics,best_results,start):
    ...
    for i, (images, visits, target) in enumerate(train_loader):
        ...
        message = '%d %s %5.1f %6.1f      |   %0.3f  %0.3f  %0.3f  | %0.3f  %0.3f  %0.4f   | %s  %s  %s |   %s   |   %.2fdB' % ( \
                config.start_fold,"train", i/len(train_loader) + epoch, epoch,
...
def training(train_data_list, val_data_list, test_files, fold):
    ...
    if config.start_epoch!=0:
        checkpoint_loss = torch.load('checkpoints/best_models/0626_debug_fold_'+str(config.start_fold)+'_model_best_loss.pth.tar')
        model.load_state_dict(checkpoint_loss["state_dict"])
        # #test(test_loader, model, fold, checkpoint_loss, 'best_loss', False)

        checkpoint_acc = torch.load('checkpoints/best_models/0626_debug_fold_'+str(config.start_fold)+'_model_best_acc.pth.tar')
    ...
    if resume:
        ...
        ### ---------- train loop ----------------
        if fold==config.start_fold:
            print("fold==",config.start_fold)
...
if __name__ == "__main__":
    print("fold ",str(config.start_fold))
    main()
