...
def evaluate(val_loader,model,criterion,epoch,train_metrics,best_results,start):
    ...
    with torch.no_grad():
        ...
        #log.write("\n")
    return [acc.avg, losses.avg, f1.avg]
def test(test_loader, model, fold, ckpt, best_what, if_gen_txt,index):
    ...
    model.eval()
    print("test fold")
    for i, (image, visit, _) in enumerate(test_loader):
        with torch.no_grad():
            ...
            if len(image.size()) == 5:
                ...
                y_pred, _ = model(image,visit)
                preds_temp=torch.nn.functional.normalize(y_pred)

            y_pred=F.softmax(y_pred).cpu().data.numpy()
            predicts.append(y_pred)
            preds_temp_npy=preds_temp.cpu().data.numpy()
            predicts_csv.append(preds_temp_npy)
        if i%200==0:
            print(i)
    print("end")
    pred_npy = np.concatenate(predicts)
    pred_csv_npy = np.concatenate(predicts_csv)
    print(pred_csv_npy.shape,"shp")
    save_name = '%s_val_%.4f_fold%d_%s.npy'%(ckpt["model_name"], ckpt["best_acc"], fold, best_what)
    save_path = os.path.join(save_dir, save_name)
    np.save(save_path, pred_npy)
    np.save(os.path.join(save_dir, "VAL_csv_0716_"+str(index)+"_"+best_what+".npy"), pred_csv_npy)
    if if_gen_txt:
...
def training(train_data_list, val_data_list, test_files, fold):
    ...
    # val_data.sort()
    val_csv="preliminary/val.csv"
    val_data = pd.read_csv(val_csv)
    val_gen = MultiModalDataset(val_data, config.train_data, config.train_vis, augument=False, mode="val")
    val_loader = DataLoader(val_gen, 256, shuffle=False, pin_memory=True, num_workers=4)
    test_gen = MultiModalDataset(test_files, config.test_data, config.test_vis, augument=False, mode="test")
    test_loader = DataLoader(test_gen, 512, shuffle=False, pin_memory=True, num_workers=4)
    ...
    for index in [1]:
        print(index)
        checkpoint_loss = torch.load('checkpoints/best_models/0626_debug_fold_'+str(index)+'_model_best_loss.pth.tar')
        model.load_state_dict(checkpoint_loss["state_dict"])
        test(val_loader, model, fold, checkpoint_loss, 'best_loss', False,index)

        checkpoint_acc = torch.load('checkpoints/best_models/0626_debug_fold_'+str(index)+'_model_best_acc.pth.tar')
