...
def main():
    ...
    print("load pretrained model from model 1 _____adam2")
    state_dict1 = torch.load('model1/BDXJTU2019_SGD_12_6000.pth')
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        #if is_best:
        if epoch%1 == 0:
            1#torch.save(model.state_dict(), 'model1/BDXJTU2019_SGD_' + repr(epoch) + '.pth')
...
def train(Dataloader,model, criterion, optimizer, epoch):
    ...
    for i, (input_img, input_vis, anos) in enumerate(Dataloader):
        ...
        if (i+1)%2000==0:
            torch.save(model.state_dict(), 'model1/BDXJTU2019_SGD_' + repr(epoch)+"_"+str(int(i+1))+ '.pth')
        if (i+1) % 200 == 0:
            print('Epoch: [{0}][{1}/{2}]\t'
