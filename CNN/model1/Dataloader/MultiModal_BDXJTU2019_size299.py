...
class MM_BDXJTU2019(data.Dataset):
    def __init__(self, root, mode, transform = None):
        ...
        self.root = root
        self.mode = mode
        with open(os.path.join(self.root, mode + '.txt')) as f:
            reader = f.readlines()
        ...
        for i in range(10):
            ...
            for file in files:
                self.npy2path[file[:-4]]=path0+str(i)+"/"+file
    ...
    def __getitem__(self, index):
        ...
        Img = cv2.resize(Img, (299, 299))        #Img = cv2.resize(Img, (331, 331))
        Img = Img[:, :, (2, 1, 0)]
...
class MM_BDXJTU2019_id(data.Dataset):
    def __init__(self, root, mode, transform=None):
        # mode can be valued as "train", "val", "trainval"
        self.root = root
        self.mode = mode
        ...
        self.normal = tr.Compose([
            tr.ToPILImage(),
            tr.ToTensor(),
            tr.Normalize(mean=(0.4685483813116096, 0.538136651819416, 0.6217816988531444),
                         std=(0.1016119525359456, 0.0900060860845122, 0.08024531900661314))
        ])
        self.transform = transform
        path0 = '../final/npy/0_9/'
        self.npy2path = {}
    ...
    def __getitem__(self, index):
        ...
        Img = cv2.resize(Img, (224, 224))  # Img = cv2.resize(Img, (331, 331))
        Img = Img[:, :, (2, 1, 0)]
        ...
        return Input_tensor, visit_tensor, Anno,item[-14:-8]
...
class BDXJTU2019_test(data.Dataset):
    def __init__(self, root):
        self.root = root
        ####
        self.ids = os.listdir(os.path.join(self.root, 'test_image'))
        self.ids.sort()
        self.normal = tr.Compose([
                           tr.ToPILImage(),
                           tr.ToTensor(),
    ...
    def __getitem__(self, index):
        item = self.ids[index]
        Img = cv2.imread(os.path.join(self.root, 'test_image', item))
